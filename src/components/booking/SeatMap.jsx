...
export default function SeatMap({ concert }) {
    return (
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg">
            <div className="mb-4 p-3 bg-gray-800 text-white text-center rounded-lg font-bold text-lg shadow-inner">استیج</div>
            <div className="space-y-3">
                {concert.seatMap.map((row) => (
                    <div key={row.row} className="flex items-center justify-center gap-2 sm:gap-3">
                        ...
                        <div className="flex-grow grid grid-cols-12 gap-1 sm:gap-2">
                            {row.seats.map(seat => <Seat key={seat.id} seat={seat} />)}
                        </div>
                    </div>
                ))}
            </div>
            <div className="mt-6 flex justify-center items-center gap-x-6 text-sm">
